...
const displayCharacters = (characters) => {
    ...
    // Color 'bounding-block' class
    const theme = localStorage.getItem('theme');
    const boundingBlocks = document.getElementsByClassName("bounding-block");
    if (theme === 'dark') {
        for (let block of boundingBlocks){
            if (block) block.classList.add('light-mode');
        } 
    } else {
        for (let block of boundingBlocks){
            if (block) block.classList.remove('light-mode');
        } 
    }
